...
type
  TfrmImprimir = class(TForm)
    ...
    procedure chkTerminadoClick(Sender: TObject);
  private
...
var
  ...
  Campos: array[0..5] of string = ('Nombre', 'Producto', 'Recibido', 'Interna', 'Entrega', 'Empleado');
  DBNames: array[0..5] of string = ('I.ITE_Nombre', 'O.Producto', 'O.Recibido', 'O.Interna', 'O.Entrega', 'O.Nombre');
...
{  if GridView1.RowCount = 1 Then
        begin
                GridView1.Cell[0,GridView1.SelectedRow].AsString := '';
                GridView1.Cell[1,GridView1.SelectedRow].AsString := '';
                GridView1.Cell[2,GridView1.SelectedRow].AsString := '';
                GridView1.Cell[3,GridView1.SelectedRow].AsString := '';
                GridView1.SelectCell(0,0);
        end
  Else}
...
procedure TfrmImprimir.GridView1AfterEdit(Sender: TObject; ACol,
  ARow: Integer; Value: String; var Accept: Boolean);
begin

{if (ACol = 0) and (GridView1.Cell[0,ARow].AsString = '' ) then
        GridView1.Cell[0,ARow].AsString := '=';

if (ACol = 0) and (GridView1.Cell[3,ARow].AsString = '' ) then
        GridView1.Cell[3,ARow].AsString := 'Y';
 }
end;
...
procedure TfrmImprimir.chkTerminadoClick(Sender: TObject);
begin
deTerminado1.Enabled := chkTerminado.Checked;
deTerminado2.Enabled := chkTerminado.Checked;
end;
